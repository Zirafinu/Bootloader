...
namespace gzip
{
  struct Header
  {
    ...
    std::array<uint8_t, 2> magic_bytes;
    uint8_t method;
    Flags flags;
    uint32_t timestamp;
  ...
  {
    return static_cast<Header::Flags>(static_cast<uint8_t>(lhs) &
  ...
  public:
    ...
    Inflate(can_read_t can_read, read_t read, write_t write, read_old_t read_old) : can_read(can_read), read(read), write(write), read_old(read_old)
    {
    }
