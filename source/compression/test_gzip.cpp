...
const std::array<uint8_t, 11> in_memory = {0x2b, 0x49, 0x2d, 0x2e, 0xd1, 0x51,
                                           0x28, 0x01, 0x92, 0x5c, 0x00};
...
uint8_t read_old_out(std::size_t offset) {
  return *(out_memory_write - offset);
}
} // namespace
...
int main() {
  ...
  { // a compressed binary image
    ...
    auto const h = gzip::Header::parse(std::begin(bootloader_bin_gz),
                                       std::end(bootloader_bin_gz));
    ...
    if (out_memory_write != uncompressed.end())
      return 5;
    if (std::memcmp(bootloader_bin, uncompressed.data(), uncompressed.size()) !=
        0)
      return 6;
  }
  return 0;
